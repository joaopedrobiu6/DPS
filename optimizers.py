...
def optimization_function_scipy(objective):
    return minimize(objective, a_initial, jac=True, method='L-BFGS-B', options={'maxiter': max_nfev_optimization, 'gtol':tol_optimization}, tol=tol_optimization)
...
def optimize_with_scipy(results_path):
    ...
    # result = minimize(objective, a_initial, jac=True, method='L-BFGS-B', options={'maxiter': max_nfev_optimization, 'gtol':tol_optimization}, tol=tol_optimization)#, callback=callback_minimize)
...
def optimize_with_pytorch(results_path):
    ...
    # optimizer = torch.optim.LBFGS(ode_system.parameters(), lr=learning_rate_torch)
    ...
    #     if step > 4 and (losses[-2] - losses[-1])/losses[-2] < tol_optimization:
    #         break
