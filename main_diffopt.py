...
from params_and_model import tmin, tmax, nt, a_initial, initial_conditions, model, solver_models, variables
...
def main(results_path='results'):
    ...
    for i in range(4):
        ...
        if i < 3:
            for j in range(len(solver_models)):
                axes[row, col].plot(t, initial_solutions[j][:, i], label_styles[j][0], label=f'{solver_models[j]} Initial')
                axes[row, col].plot(t, optimized_solutions[j][:, i], label_styles[j][1], label=f'{solver_models[j]} Optimized')
            ...
            for j in range(len(solver_models)):
                ax_3d.plot(initial_solutions[j][:, 0],initial_solutions[j][:, 1],initial_solutions[j][:, 2], label_styles[j][0],label=f'{solver_models[j]} Initial')
                ax_3d.plot(optimized_solutions[j][:, 0],optimized_solutions[j][:, 1],optimized_solutions[j][:, 2], label_styles[j][1],label=f'{solver_models[j]} Optimized')
    ...
    [plt.plot(np.log(loss_data[i]), label_styles[i][1], label=solver_models[i]) for i in range(len(solver_models))]
